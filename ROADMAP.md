...
> **Today:** OCI digest pins the bundle, audit log records every intent mutation — both exist, not stitched on the publish history page. "What changed between v8 and v9" is answerable today only by manually time-windowing the audit log against publish timestamps. **Mechanism (reconciled with Spike 30, 2026-08-21):** the changeset is a **content-diff of the two published artifacts** — pull both by immutable OCI digest → normalize to `canonicalNode` → compare, per `orbId`, field-level, flat rows. **Same normalize+compare core as Spike 30** (`docs/reference/DGRAPH.md` § "Comparing two graph snapshots"); Spike 30 diffs live-blue vs last-published, Spike 25 diffs two *published* artifacts (two baseline inputs, no live side). **Do NOT use the audit-cursor-window as the diff source** — a `dropAll` restore / seed / out-of-band write between publishes is invisible to it (why Spike 30 rejected it). **Audit's role = who/when enrichment only:** join the diff to the audit log by orbId for actor/timestamp per change; optionally enrich the publish audit event with `{oci_digest, version, entry_count}` for provenance on the publish-history page. **Explicit non-goal:** do NOT log all N resource IDs per publish — the OCI artifact already IS the authoritative manifest; duplicating ~780 IDs per publish would inflate the audit log by ~285K rows/year for data that's already digest-pinned. **Reference systems:** NetBox per-object change log, Argo CD sync history, Helm `helm history` + `helm diff`, Terraform Cloud plan output — all share the shape: versioned anchor + changeset-since-previous-anchor. **Gitops critique to answer:** (a) rollback = DGraph backup/restore tied to publish digest (we have this); (b) "diff v8..v9" = content-diff of the two artifacts (this spike, shared Spike 30 core); (c) why not git? — intent lives in DGraph because traversal queries are the primitive, git is bad at that shape; (d) "review changeset before Publish" = **Spike 30** (read-only preview) + **Spike 31** (selective revert + guarded Apply). Triggered by 2026-06-23 conversation.
...
**Spike 31 — AEP change-management: guarded Apply + selective revert + attribution** · *Designed; not yet implemented (depends on Spike 30)*
As a data center owner, how do I review accumulated multi-user (PIM) changes, revert individual ones, and Apply — without un-reviewed intent reaching the edge?
> **Design: [docs/spikes/spike-31-change-management-design.md](docs/spikes/spike-31-change-management-design.md).** The write half of the AEP change-management flow (Approach 2); Spike 30 is the read half. Builds on Spike 30's preview (`docs/reference/OCI.md` § "Export preview"; configbundle `.local/design-change-management.md` Option C). **Three parts:** (1) **Selective revert** — reset chosen nodes/fields to the baseline value the preview already returns; minimal path reuses the existing `update{Type}` mutations, optional batch `POST /api/v1/export/revert`; PIM/authz-gated + audited. (2) **Guarded Apply** — `expectedContentHash` on `POST /api/v1/export` (**shipped 2026-08-24** — see `OCI.md` § "Guarded Apply"): abort `409` if intent moved under the reviewer between preview and Apply; the concurrent-PIM-writer race is the trigger. Apply = export **and** publish. (3) **Attribution** — join the diff to the audit log by orbId (`GET /api/v1/audit-log?orbId=…`) for "who changed this"; audit = who/when, diff = what. **No new state:** no field-history table, no staging layer, no per-entity version field, no PIM logic in Orbital (role enforced via `RequireRole`; PIM is IdP-side). Triggered by 2026-08-21 AEP + configbundle design session.
