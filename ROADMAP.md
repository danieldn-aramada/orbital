...
- **2026-08-26** — Artifact compare UI: Publish History tab bar with route-backed Compare (`?from=&to=` linkable), reusing the preview renderer; `?dc=`/`status`/`limit` filters on `/api/v1/oci/artifacts` (also fixes the capped-list defect and answers configbundle's version-lag ask).
