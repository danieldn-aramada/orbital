...
| 31 | Reject inline single-entity mutations (server-managed metadata) | Should the proxy auto-stamp inline mutations, or require the variable form and reject inline? | — | Not started (design settled) | **Decision (Opus design 2026-07-28): Option D — reject inline single-entity `update{Kind}` mutations with a descriptive 400; do NOT rewrite the query string.** **Problem:** the proxy stamps `version` + `updatedBy`/`updatedAt` (authenticated identity + server clock) only on the **variable-based** path — the before-fetch gate reads `req.Variables["orbId"]`/`["id"]`, so an inline `filter:{orbId:{eq:"…"}}` is invisible to it and stamping is **silently skipped** (version not bumped, audit fields null). Verified empirically 2026-07-28: same mutation with `$orbId`/`$set` vars → version 2 + stamped; inline → version 1 + null. **Why D over rewriting (A `gqlparser` AST / B extract-inline→vars / C regex string-inject):** every mutation flows through `Handle` (UI, orbctl, cb-bundler) — A/B/C add query-*string* manipulation to that load-bearing path where a bug could corrupt *working* mutations; **D only adds a rejection branch on a shape that already silently misbehaves, so it cannot regress any working path.** Our own clients already use the variable form (UI `configitem-editor.js:84`, orbctl `patch_dc.go`), so D breaks ~zero real traffic — only hand-written GraphQL and the (now-fixed) cheatsheet. D converts a silent-correctness bug into a loud, self-documenting error, matching the codebase's document-and-defer style (`isMutation` already declined `gqlparser`). **Scope — documented pass-throughs, NOT rejected:** bulk `in:[…]`, non-orbId filters, compound multi-mutation docs, and inline `add(input:[…])` literals (UI/orbctl always use `$input`). **Implement:** inline-single-entity detector after `resourceType` resolves in `graphql.go` (reuse `orbIdFilterRe` + `stripCommentsAndStrings` to avoid string/comment false-triggers); 400 JSON body pointing at `docs/api-cheatsheet.md`; tests for must-stamp / must-reject / must-pass-through / false-trigger cases. **Upgrade path if external inline clients ever appear: Option B** (lift the inline object into a synthetic `$__set` variable, then reuse the safe map-injection). **Already shipped separately (NOT this spike):** server-side stamping on the variable path + removal of client-side stamping + cheatsheet variable form. Triggered by the 2026-07-28 galleon-backup-svc `updatedAt` report. |
