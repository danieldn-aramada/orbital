...
- **2026-06-18** — Cluster schema polymorphism shipped: `interface KubernetesCluster` (universal: `kubernetesVersion`, `controlPlaneEndpoint`, `dataCenter`, `cni`, `environment`, `nodes`) + `type EksaKubernetesCluster implements KubernetesCluster & ConfigItem` (provider-specific: `clusterType`, `tinkerbellIP`) + `type KubernetesNode implements ConfigItem` (role, cluster, server); old `KubernetesCluster` concrete type + `EksaConfig` sidecar removed; audit map + cascade delete updated for interface-level + concrete-type mutations. **Publish History** rename: `signed-artifacts` → `publish-history` end-to-end (menu, URL, page template, page title, e2e test); `RegistryArtifact.nextTagForJob` now filters `status=completed` so failed publishes no longer burn tag numbers. **Orb import partial status**: new `partial` lifecycle for graph-applied-but-dispatch-failed imports; yellow tag in import-history; orb.js status banner handles `done`/`partial` as terminal (was stuck at "Pending…"); orb dispatch errors surface inline in layers modal instead of hover-only tooltips. **Audit log orbId fix**: `acceptDivergence`/`rejectDivergence`/`dismissDivergence` events now record the underlying resource's real orbId (e.g. `colo:CWJHDX3-idrac`), not the synthetic `<orbId>:<field>` compound that polluted the orbId column. **cb-bundler single base URL**: collapsed `ORBITAL_GRAPHQL_URL`+`ORBITAL_API_URL` into `ORBITAL_BASE_URL`; cb-bundler derives `/graphql` and `/api/v1/...` internally via `graphqlURL()`/`divergencesURL()`; AKS deploy uses `$(ORBITAL_BASE_PATH)` interpolation. **Startup preflight removed**: deleted `internal/startupcheck` package — the bundler-and-consumer preflight created a startup race with the in-pod sidecar (1 restart per pod cold-start) and provided asymmetric protection (boot-only, no runtime detection); replaced with a configured-URLs info log. **Env var rename**: `ORB_POLL_INTERVAL` → `ORB_OCI_POLL_INTERVAL` (matches `ORB_OCI_*` namespace), default `60s` → `30s`. **Per-row Publish on Divergence Reports**: outlined "Publish" button per DC row triggers export+publish-modal flow without page navigation; server-side `AlreadyPublished` flag (computed: `max(registry_artifact.completed_at WHERE status=completed) > max(resolution.decided_at)` for the DC) renders the button as permanent green "Published v17" disabled state, enforcing cross-session one-shot publish semantics. **Submit-decisions modal copy**: jargon-free explanations for Reject ("will be lost") and Ignore ("edge admin retains ownership") — dropped "force-apply"/"cb-manifest"/"next bundle" implementation terms. **Layers-modal scroll guard**: publish-history table now bounded by `max-height: calc(100vh - 420px)` with sticky `<thead>`. **IPv4 sort**: shared `dtIPv4Render` + `ipv4SortKey` helpers in `shared.js`; applied to `#server-list-table` and `dc-servers-table-*` OOB IP columns — `.100` no longer sorts before `.41`. **ADR-009 orbital-side cleanup**: `internal/divergenceingest/store.go` loop-close path wrapped in a per-entry transaction (resolution+entry delete atomically) + new `closeIgnoreOnHandback` audit event when an `action=ignore` resolution loop-closes (signaling edge admin released their SSA claim and cb-controller's ReclaimController restored intent); `DIVERGENCE.md` updated to reflect that Ignore now loop-closes via edge handback, not "persistent until manually removed". **Naming hygiene**: `divergence.Snapshot`/`SnapshotKey`/`applySnapshot`/`LastSnapshotPublishedAt` → `Report`/`ReportKey`/`applyReport`/`LastReportPublishedAt` end-to-end (production term matches the existing UI label "Divergence Reports" + matches Puppet/driftctl industry convention; "snapshot" was overloaded with DB-snapshot meaning). **Divergence ingester DC discovery**: switched from `registry_artifacts` event log to DGraph `queryDataCenter` — eliminates the stale-row-per-DC bug from DGraph-UID → orbId migration history (was applying the same snapshot 4× per DC under different identifiers). **Bundler bug fixes**: `ORBITAL_OIDC_*` env var names corrected in `deploy/base/deploy.yaml` (were `ORBITAL_TENANT_ID`/`ORBITAL_CLIENT_ID`/`ORBITAL_CLIENT_SECRET`, didn't match what cb-bundler reads); `ORBITAL_GRAPHQL_URL` + `ORBITAL_API_URL` include base path via `$(ORBITAL_BASE_PATH)` interpolation. **`bundler.ParseSpec` extracted** to `internal/bundler/client.go` for cross-package reuse (was private in `handler/oci.go`). Memory: new `feedback_trace_enum_consumers.md` rule — when adding enum/status values, grep every existing value across Go + JS + templates and verify each consumer site handles the new value before declaring done (prevents the silent "Pending…" stuck-banner class of bug that hit on `partial` rollout).
- **2026-06-16** — Divergence semantics hardened across both repos. **Orbital**: per-FIELD MVCC pin on `divergence_resolutions.intended_at_version` (Accept=entry.version+1 post-mutation; Reject/Ignore=entry.version); `List(?action=accept|reject)` queries DGraph for each field's current value and excludes stale resolutions — bundler never emits a takeover that would force-apply a contradictory value; per-FIELD (not per-ConfigItem) so sibling Accept doesn't invalidate Reject on same IdracSettings. New tests `TestList_ActionFilter_ExcludesStaleResolution` + `TestList_ActionFilter_BatchAcceptAndRejectOnSameConfigItem`. **Orb importer fresh-install bug**: `applySchema` now does `MkdirAll(DataDir)` before writing `schema.graphql` — was failing silently on first import because Import()'s MkdirAll happened later. Pinned by `TestImporter_Import_WritesSchemaFileOnFreshInstall`. **Orb poller hardened**: `highestVersionTag` parses `v<N>` numerically (v10>v9), ignores non-version tags (`latest`, cosign sigs) instead of letting them become the candidate; transient registry errors keep last-known `availableVersion` and record `lastPollErr` in the snapshot for diagnostics. **Orb state**: hydrates `lastImport` from import-history.json on startup. **Orb status page UX**: "New version available" notification moved above the status grid with link to `/import` (not a direct trigger button); layers-modal gains per-layer Dispatch column showing status code + consumer name. **Configbundle (parallel repo)**: new `spec.ignored[]` CRD field replaces bundler's old nil-the-field omission — intent value stays in `spec.servers[N]` so divergence-reporter can keep comparing and surfacing the override every cycle; Ignore is now a STANDING instruction (no longer a suppression mechanism), only exits via local-admin release OR Accept/Reject re-decision. cb-controller's `omitAdminOwnedFields` gained four-way decision (ignored/takeover/value-match/value-differ). Reporter dropped `intendedAbsent` skip. ADR-008 refinements documented. Invariant scope clarified: "no co-ownership of VALUE fields" — SSA's listMapKey/entry-presence/struct-wrapper co-ownership is structural and inert. **Orbital UX polish**: divergence-reports rows collapsed by default (no sessionStorage persistence — newly-arrived reports never auto-expand from prior session); Reject confirmation copy rewritten to drop "deployment layer"/"propagated" jargon; export status banner's redundant Download link removed (use table row); `schema/schema.graphql` header comment no longer hardcodes version (VERSION file is source of truth).
