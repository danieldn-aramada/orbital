...
| 24 | Divergence MVCC + storage split (rejected DGraph move) | Close the report→resolve race condition by capturing ConfigItem version at intake and checking at Accept. Storage stays in PostgreSQL. | Daniel | In progress | **Decision settled 2026-06-14:** divergence/resolution stay in PostgreSQL. Graph-native move (`divergence-as-graph-state.md`, now superseded) was rejected — export-pipeline contamination is the killer argument (orb shouldn't receive resolutions; stripping them at export is the same vocabulary leak just relocated). MVCC plan in `docs/plans/divergence-storage-mvcc.md` (concrete steps 1–6 ready to execute). Open: API unification (federated GraphQL) deferred — separate spike when the "thin proxy vs gateway" architecture question is settled. |
