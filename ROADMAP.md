...
- **2026-06-10** — Makefile slim (28→22 targets, grouped help); CLI renamed `orbital-cli` → `orbctl`; consolidated single Dockerfile with `orbital`/`orb` targets; image-based `make release-check` replaces local-only smoke (validates production code paths, not dev backends); `make release-check VERSION=v0.0.x` dual-tags images for visibility; orb Dockerfile shipped (multi-stage with dgraph baked in + `gcompat`/`libc6-compat` for glibc-on-alpine — Spike 14 progress); `/api/v1/inventory` REST endpoint deleted (UI now uses GraphQL `queryConfigItem`); Swagger `@BasePath` cleanup + version wired to `version.Version`; `.gitattributes` linguist hints (Go now #1 on GitHub); orb `/healthz` registered; `backup_schedules` test rot cleaned; dev-only `DockerBackend` / `DockerRestoreBackend` removed (validation now exercises only the production code path); `ORBITAL_COOKIE_SECURE` env var decouples Secure cookie flag from `ORBITAL_DEV` (AKS-dev login unblocked over HTTP); device-code page moved from `partials/` → `pages/`, reuses shared `head.gohtml` (cache-bust + future-proof); clipboard copy on device-code page hardened with hidden-textarea + `execCommand` fallback for non-secure contexts (AKS dev HTTP); `TestDeviceCodeStart_RendersUserCode` strengthened to pin doctype, head include, and `?v=` cache-bust (locks the template-naming foot-gun); `edge-up`/`edge-down` make targets restored for testing orb against AKS upstream via zot mirror; `CONTRIBUTING.md` overhauled to reflect new commands; CHANGELOG.md deleted in favor of `## Recent accomplishments` section; `/wrap-up` skill tightened to route decisions to domain files first, CLAUDE.md last
