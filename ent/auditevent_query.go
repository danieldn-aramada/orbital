...
type AuditEventQuery struct {
	...
	withResources     *AuditEventResourceQuery
	withResourceTypes *AuditEventResourceTypeQuery
...
func (_q *AuditEventQuery) QueryResources() *AuditEventResourceQuery {
	query := (&AuditEventResourceClient{config: _q.config}).Query()
...
func (_q *AuditEventQuery) QueryResourceTypes() *AuditEventResourceTypeQuery {
	query := (&AuditEventResourceTypeClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *AuditEventQuery) WithResources(opts ...func(*AuditEventResourceQuery)) *AuditEventQuery {
	query := (&AuditEventResourceClient{config: _q.config}).Query()
...
func (_q *AuditEventQuery) WithResourceTypes(opts ...func(*AuditEventResourceTypeQuery)) *AuditEventQuery {
	query := (&AuditEventResourceTypeClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(auditevent.FieldOperations).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.AuditEvent.Query().
//		Select(auditevent.FieldOperations).
//		Scan(ctx, &v)
...
func (_q *AuditEventQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*AuditEvent, error) {
	...
	if query := _q.withResources; query != nil {
		if err := _q.loadResources(ctx, query, nodes,
			func(n *AuditEvent) { n.Edges.Resources = []*AuditEventResource{} },
			func(n *AuditEvent, e *AuditEventResource) { n.Edges.Resources = append(n.Edges.Resources, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withResourceTypes; query != nil {
		if err := _q.loadResourceTypes(ctx, query, nodes,
			func(n *AuditEvent) { n.Edges.ResourceTypes = []*AuditEventResourceType{} },
			func(n *AuditEvent, e *AuditEventResourceType) {
				n.Edges.ResourceTypes = append(n.Edges.ResourceTypes, e)
			}); err != nil {
			return nil, err
		}
	}
...
func (_q *AuditEventQuery) loadResources(ctx context.Context, query *AuditEventResourceQuery, nodes []*AuditEvent, init func(*AuditEvent), assign func(*AuditEvent, *AuditEventResource)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(auditeventresource.FieldAuditEventID)
	}
	query.Where(predicate.AuditEventResource(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(auditevent.ResourcesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "audit_event_id" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *AuditEventQuery) loadResourceTypes(ctx context.Context, query *AuditEventResourceTypeQuery, nodes []*AuditEvent, init func(*AuditEvent), assign func(*AuditEvent, *AuditEventResourceType)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*AuditEvent)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(auditeventresourcetype.FieldAuditEventID)
	}
	query.Where(predicate.AuditEventResourceType(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(auditevent.ResourceTypesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
