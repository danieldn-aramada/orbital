...
Use `orbital-cli` to get an access token via PKCE. One interactive login, then the token sits in a local file you can `cat`.

```bash
# One-time interactive login (opens browser, redirects to localhost)
orbital login

# Grab the access token and call orbital
TOKEN=$(jq -r .access_token ~/.orbital/credentials.json)
...
Token lasts ~1 hour. Re-run `orbital login` when it expires. Fine for poking, scripting, demos. Not for long-running services.
