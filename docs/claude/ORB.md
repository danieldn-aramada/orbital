...
- **`POST /api/v1/import/artifact` is the full import pipeline** — always registered. Accepts a complete multi-layer OCI artifact zip. Decomposes layers, dispatches non-graph layers to registered consumers, imports graph layers to DGraph. Symmetric reverse of orbital's enricher pipeline.
