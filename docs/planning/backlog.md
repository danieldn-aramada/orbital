...
| [Spike 26 — Multiple identity providers](../spikes/spike-26-auth-providers.md) | **Shipped**, one validation gap | `ORBITAL_AUTH_PROVIDERS` — a list of providers, each mapping its own group claim to orbital roles. Decisions live in [AUTH.md](../reference/AUTH.md); the spike file is now only the upstream survey and deliberation. Remaining: end-to-end check of group-driven roles with a real user token, via `deploy/local/orbital.env`. |
