...
| Key user identities on `(iss, sub)`, not email | Not started | Orbital keys `users` by email, which OIDC states is mutable, reusable across end-users, and falsifiable — "the only guaranteed unique identifier is the combination of the iss claim and the sub claim". Consequence today: one address cannot be shared across providers, so a second provider asserting it is refused `IDENTITY_CONFLICT`. The conventional fix is an identity-link table keyed `(issuer, subject)` — Keycloak `federated_identity`, Auth0 account linking, django-social-auth `usersocialauth` — giving one person N provider links. **Carries a UI question**: with N links, "role from &lt;issuer&gt;" is ambiguous — the users page must say which provider set it *and when*, or it will confidently name the wrong one. → [AUTH.md](../reference/AUTH.md) |
| CLI authentication, redesigned | Not started | `internal/orbauth/auth.go:74,168` builds `https://login.microsoftonline.com/{tenant}/oauth2/v2.0/...` by string concatenation, so `orbctl login` breaks now that orbital trusts only Keycloak. Same vendor-lock pattern that got device-code removed. Fix is OIDC discovery instead of URL construction. Rescoped 2026-09-22 from "add OIDC discovery" to a design question, since nobody uses it: who issues the credential the CLI holds — a **public IdP client** (kubectl/gh; zero orbital code, but every adopter must register a client), **orbital itself** (Vault/Teleport/ArgoCD; no adopter setup, but orbital becomes an issuer), or **personal access tokens** (GitHub/GitLab/NetBox; IdP-independent and individually revocable). The name is also in question — `orbctl` reads as "control orb" while it targets both. |
