...
| Item | Sev | Notes |
|---|---|---|
| `BeforeFields` is hand-maintained beside `FormFields`, and disagreement is SILENT | Hi | Drop a field from one and the mutation still succeeds while the audit event carries no `changes` at all — no error, nothing in the log (demonstrated on `Rack.uHeight` 2026-09-23). Both strings are mechanically derivable: `id orbId name version` + own FormFields + `<childField> { child FormFields }` per owned child — verified identical for `Server`'s `idracSettings`/`serverMaintenance`. **Generate it rather than testing that two lists agree.** `configitems/registry.go`; the existing `TestParity_WithLegacyHandMaintainedValues` is the migration safety net, but it snapshots a deleted legacy map so `Rack`, `IPAddress` and `IdracSettings` are uncovered and every new type starts uncovered. |
...
| Config editor silently discards unknown JSON keys | Hi | Types a key the registry doesn't list and you get a success toast, a `version` bump and an audit row — with the value dropped. A typo'd key is indistinguishable from a real edit. Reject unknown keys with a 400 naming them, as `ORBITAL_AUTH_PROVIDERS` now does at startup. Hit live 2026-09-22 with `serialNumber`. |
| Registry field lists are hand-maintained and unverified against the schema | Med | `configitems/registry.go` `FormFields`/`BeforeFields` must be updated by hand for every new ConfigItem field; nothing fails when they drift, so the field is silently non-editable and absent from audit diffs — the root cause of the row above. `registry_test.go` pins `BeforeFields` as a literal, mirroring the implementation, so it cannot catch this. The allowlist itself is legitimate (`orbId`, `version`, `createdAt` and edges must stay non-editable) — what is missing is a test parsing `schema/schema.graphql` and asserting the lists match its scalars minus an explicit exclusion set. → [add-configitem.md](../playbooks/add-configitem.md) |
