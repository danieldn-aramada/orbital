...
2. **`writeError(c, status, code, msg, hint)`** — direct-writes the envelope. Used where a site
   ...
   ⚠️ **It returns `c.JSON`'s error, which is `nil` on success** — never hand that value back
   as a caller's only signal. See the Settled Decision below.
...
| `MVCC_CONFLICT`              | 409 | `graphql.go` `version` pre-flight AND the compare-and-swap that matched no row; change-request create/amend/merge; `DELETE /config-items` `?version=`; `export.go` `Trigger` `expectedContentHash` check | State changed since the caller's read; reload and retry. Two scopes, same semantics: `version` guards **one record**, `expectedContentHash` guards **a whole DC subgraph** before publish (see `OCI.md` § "Guarded Apply"). Set explicitly via `writeError` (NOT the 409 default) — a bare 409 would collide with `CONFLICT`, which the export endpoint also returns for "already in progress", and clients could not tell "re-review your diff" from "wait and retry". |
| `WRITE_CONTENTION`           | 503 | `graphql.go` write path | DGraph aborted the transaction repeatedly — another write kept landing on the same records. **Distinct from `MVCC_CONFLICT` on purpose:** that means the caller's data is stale and they must re-read; this means the same request is still valid and the store was busy. Different remedy, so a different code. Orbital retries a bounded number of times first; this is only raised when that does not clear it. More likely since the version predicate moved inside the write — two writers filtering the same predicate contend where an unfiltered write would not. |
...
| `BAD_USER_INPUT`             | 400 (also 405/422) | REST validators; Echo bind/routing errors; `graphql.go` malformed `version` | Malformed or missing request fields. The `codeForStatus` default for any bare 4xx without a more specific mapping. |
| `VARIABLE_FORM_REQUIRED`| 400 | `graphql.go` inline-selector guard (Spike 31, live) · approval gate (`approval_gate.go`) | **Two causes, one remedy — send variables.** (1) A single-entity `update{Kind}` did not name exactly one row through variables, so the proxy can't stamp `version`/`updatedAt`/`updatedBy`; kill switch `ORBITAL_INLINE_SELECTOR_REJECT=false`. **The variable NAMES are irrelevant** (since 2026-09-14): `update{Kind}($serverOrbId, $patch)` is accepted and stamped exactly as `($orbId, $set)` is. What is still refused is a shape that cannot be pinned to one row — an inline literal selector or inline `set`, an `in:` list, two different orbIds, or a whole filter behind `filter: $var` (readable by the gate, but with nowhere to splice the CAS predicate). If this fires on a request that *is* in variable form, the bug is in resolution, not in the caller — do not tell them to rename their variable. (2) An `add{Kind}` with a fully inline `input` while approval policies are in force: the gate resolves the governing policy from the entity's orbId and cannot read one out of an inline literal, so it **fails closed** rather than letting the write skip every policy. Cause (2) has no kill switch — disabling it would make the gate optional for anyone willing to inline their literals. Raised only when at least one enabled policy exists, so ungated deployments never see it. |
| `APPROVAL_REQUIRED`          | 403 | approval gate in `GraphQL.writeToDGraph` (Spike 36) | The mutation targets a namespace/type covered by an enabled `approval_policy`, and the caller's role is not in that policy's `bypass_roles`. **Nothing was written.** The `hint` points at `POST /api/v1/change-requests` — the refusal is a redirect, not a dead end. Raised at the shared write function so it covers BOTH `/graphql` and internal dispatch (divergence Accept); merging an already-approved change request is the one exemption. A caller whose role IS in `bypass_roles` writes through and the write is logged as a **privileged write**. |
| `NO_ROLE_MAPPED`             | —   | browser login callback (`handler.OIDC.Callback`, Spike 26) | The identity provider authenticated the user, but no group in the token matched that provider's `roleMapping`. **No user is provisioned and nothing is granted** — a mapping enumerates who may use orbital, so an unmatched caller was not enumerated. Unlike the rest of this registry this code is NOT returned as a JSON envelope: the browser flow redirects to `/?error=NO_ROLE_MAPPED` and the login modal renders the code plus `error — hint`. Same registry, different transport. |
| `IDENTITY_INCOMPLETE`        | —   | browser login callback (`handler.OIDC.Callback`) | The provider supplied no `email` claim. Orbital identifies users by email, so there is nothing to key a user row on. Also redirect-carried rather than JSON. |
| `IDENTITY_CONFLICT`          | 401 | `ResolveUser` (bearer) and the browser login callback (Spike 26) | A provider presented a valid token for an address that already belongs to a different principal — a local account, or another provider's user. **The row is not modified and the login is refused**; providers match rows they own or create new ones, and never claim existing ones. Orbital keys users by email today, so one address cannot be shared across providers — the durable fix is an identity-link table keyed `(iss, sub)` (see backlog). Bearer callers get the JSON envelope; the browser flow redirects with the code. |
| `INVALID_STATE`              | —   | browser login callback (`handler.OIDC.Callback`) | The callback's `state` did not match the one this browser stored, or the stored login attempt was already consumed. A stale tab, a resumed link, or a replayed callback. Browser transport: redirects to `/?error=INVALID_STATE`. Compared in constant time. |
| `INVALID_NONCE`              | —   | browser login callback (`handler.OIDC.Callback`) | The ID token verified — signature, issuer, audience, expiry all genuine — but its `nonce` does not match the login attempt that asked for it. **This is the replay case**, and the only check that sees it. A token carrying no `nonce` at all is refused here too. |
| `NO_ID_TOKEN`                | —   | browser login callback (`handler.OIDC.Callback`) | The token response carried no `id_token`. Orbital reads browser identity from the ID token, so the provider must grant the `openid` scope. |
| `TARGET_MISSING`             | 409 | `POST /api/v1/change-requests/:id/merge` (Spike 36) | An entity that existed when the change request was opened has since been deleted. Distinct from `MVCC_CONFLICT` on purpose: `MVCC_CONFLICT` means "re-review the diff", `TARGET_MISSING` means the entity is gone and no amount of re-reviewing fixes it. Merging anyway would recreate the entity from a partial field-delta — a successful-looking merge that silently corrupts data. The `hint` names the three ways out: close the request, `PATCH` it to drop that item, or recreate the entity and re-review. |
...
- **403** — authenticated but not permitted. `FORBIDDEN` (role too low), `APPROVAL_REQUIRED` (role sufficient, but this change class needs review — retry the same request as a change request, not as a different caller).
...
- **A client rendering only `error` throws away the useful half of a `problems[]` response.** `validationErrorResponse` carries `problems[{index, orbId, field, message, hint}]` — the per-field detail — and the changeset validator, the `version` precondition and the merge-time conflict check all populate it. Orbital's review page rendered only `body.error`, so a merge conflict read as *"state moved since you read it"* with the field, the old value and the new one all present in the response and discarded at the last step (fixed 2026-09-01, `failWithProblems` in `orbital.js`). **Always render `problems[]` when present**, and de-duplicate `hint` across entries — problems sharing a cause repeat it.

- **`writeError` returns `c.JSON`'s `nil` on success — so a validation helper must NOT hand that value back as its only signal.** A helper written as `func validate(...) error { return writeError(...) }` reports "no problem" to its caller while having already committed a 400, and the handler carries on and appends a second body to the same response. This shipped for one build of `validateTitle` in `changerequest_api.go` and produced a `400` whose body contained the error envelope *and* a successfully created change request. Return `(ok bool, err error)` and branch on `ok`, as `validateTitle` now does. Do not rely on `ErrorHandler`'s `c.Response().Committed` guard to paper over it — that stops the second *error* render, not the handler's own happy path. **Second instance, and the more expensive one — 2026-09-03:** the cascade-delete approval guard ended in `return writeError(...)`, so `if err != nil` never fired; the endpoint wrote a `409` **and deleted the cascade anyway**. Same mechanism, worse blast radius: a validation slip appends a stray body, a guard slip performs the destructive act it was added to prevent. Caught by `delete_guard_integration_test.go`, not by review — the code reads exactly like working error handling. **The rule generalises past validators: a guard must return a decision the caller inspects, and the caller writes the response.**
