...
- **Runtime DC identity is data-derived; bootstrap is deploy-time config** — `DCSlug` was removed from `orbconfig.Config`. After `drop_all` + live load, orb derives which data center it serves from imported DGraph data (one `DataCenter` node). Bootstrap is a separate concern: when `ORB_ENABLE_OCI_REGISTRY=true`, orb polls a specific OCI repository configured via `ORB_OCI_REPO` (e.g. `orbital/colo-galleon`). That path is an OCI repository selector — orb does not parse it for identity; the DC slug appearing in it is operator convention only. The registry implementation (Zot today, anything OCI-compliant tomorrow) is not orb's concern. In push-based deployments (ConfigBundle Controller), `ORB_OCI_REPO` is unused and `ORB_ENABLE_OCI_REGISTRY=false`. Do not re-add a `DCSlug` field; do not couple OCIRepo parsing to DC identity.
...
- **`orb.spec.ts` runs as a separate Playwright project** — `playwright.config.ts` declares two projects (`orbital` against `:8001` and `orb` against `:8010`); the `orbital` project excludes `orb.spec.ts` so orb tests don't run against the orbital server. `make test-e2e` runs both projects.
...
- **`DGraphBackend` interface abstracts orb's dgraph live execution** — `DockerBackend` (default, `ORB_BACKEND=docker`): `docker cp` + `docker exec` into the alpha container. `SubprocessBackend` (`ORB_BACKEND=k8s`): runs `dgraph live` as a subprocess inside the orb pod; requires the dgraph binary in orb's container image (same multi-stage copy as orbital's Dockerfile). Do not collapse the two backends — they serve genuinely different deployment contexts.
- **Orb K8s backend runs dgraph live as a subprocess, not via idle pod or exec into alpha** — no idle `dgraph-live` pod needed, no shared PVC. Orb's container image includes the dgraph binary (copied from `dgraph/dgraph:vX.Y.Z` in a multi-stage build). `dgraph live` connects to alpha via gRPC (`ORB_DGRAPH_ALPHA_GRPC`) and zero (`ORB_DGRAPH_ZERO_GRPC`). Data file path is local to the orb pod — no PVC wiring required.
- **`orbserver.New` returns `(*Server, error)`** — backend init failure is a hard startup error, not a warn-and-fallback. Do not change to a panic or silent fallback.
