...
- **Orbital is NetBox-shaped on row admission and NOT on payload — do not claim otherwise.** NetBox's `ObjectChange` stores a **full object snapshot on both sides** (`prechange_data` / `postchange_data`) and diffs at render time, so its changelog is complete for creates, deletes, and every field. Orbital stores the mutation **input** as the after-state and a **hand-declared per-type subset** as the before (`configitems.Registry.BeforeFields`), then intersects them in `computeChanges`. Three consequences, all live today: a create has no `before`, so `changes` is absent entirely; a multi-entity event cannot be attributed per node; and **a scalar missing from that type's `BeforeFields` silently produces no diff** — `StorageDevice` (`capacityBytes`, `manufacturer`, `model`, `serialNumber`, `wwn`), `NetworkInterface` (`macAddress`, `portType`, `linkSpeedMbps`, `mgmtOnly`) and `ServerConfigurationProfile` (`json`) all declare only `id orbId name version`, so editing any of those fields renders raw variables where a diff belongs. `TestBeforeFields_ReturnsForEveryRegisteredType` asserts only that the selection is non-empty and carries `orbId`+`version` — it does **not** check parity against the schema's scalars, so this class of gap is currently uncaught. Closing it is `backlog.md` § "One audit event per entity" (grain) and Spike 35 (changed-fields vs full snapshots).

- **A `DispatchMutation` caller no longer has to supply `before` — the fetch happens for every write.** *(Changed 2026-09-03; this bullet used to say the opposite.)* The before-fetch, the `version`/`updatedAt`/`updatedBy` stamping and the `ifVersion` check all live in `writeToDGraph`, the one function both entry points pass through. They used to live in `Handle`, so `DispatchMutation` got none of them and left each to the caller — and both callers forgot a different one: **merge** passed a `before` holding only `version` plus the `clear` fields, so its audit row rendered raw variables where a diff belongs; **divergence-Accept** passed a good `before` but wrote intent with no version bump, which made an Accept invisible to change-request staleness. Neither failed anything at the time. **What to know when adding a call site:** dispatch the canonical `update{Kind}($orbId, $set)` shape — the row to stamp is resolved from the `orbId` VARIABLE, so an inline selector or a `$filter` object resolves to nothing and you silently get neither the stamp nor the diff. Pass `before` ONLY when you hold a field the type's `BeforeFields` selection does not read (divergence-Accept does: it names its field at runtime); it is merged UNDER the fetch, field by field, never instead of it.
