...
- **A caller of `DispatchMutation` must supply a COMPLETE `before` — it does no auto before-fetch.** `Handle` (the client-facing `/graphql` path) fetches the entity first, which is why a direct save produces a field diff; `DispatchMutation` does not, and its doc comment says so. `computeChanges` intersects `before` with the mutation variables and drops `skipDiffFields`, so a `before` missing the fields being set yields an empty `changes` array and `events-table.gohtml` falls back to `VarSummary` — raw variables where a diff belongs. The change-request merge shipped exactly that bug: `fetchMergeTargets` read only `version` plus the `clear` fields. **When adding a new `DispatchMutation` call site, read the fields you are about to write** and pass them in `before`, scalars only — an edge value is a map and cannot be selected bare in GraphQL. **The same gap covers `version`/`updatedAt`/`updatedBy`**: `Handle` stamps them, `DispatchMutation` does not, and divergence-Accept does not stamp them either — see `debt.md` Track B, which proposes moving both the fetch and the stamping into `writeToDGraph` so no future caller can forget.
