...
- **A changeset item is an end-state payload with an OPTIONAL field-level precondition — not a delta.** `set` carries values; `before`, keyed identically, carries what the caller read. Supplied ⇒ conditional: orbital refuses with **409 `MVCC_CONFLICT`** at creation if a named field has already moved, and again at merge if one moves between review and apply. Omitted ⇒ unconditional, guarded only by the entity-level `base_hash`. This is `If-Match` (RFC 9110), `resourceVersion` inside a Kubernetes patch body, and JSON Patch's `test` op (RFC 6902) — conventional in three places, invented in none. **Not NetBox's `prechange_data`**, despite the resemblance: that is a before-image *recorded after the fact* on an `ObjectChange` row, and NetBox core has no optimistic concurrency on writes at all. Carrying a before-image and using one as a precondition are different things, and only the second is what `before` does. (Corrected 2026-09-02 — the original line claimed the NetBox lineage.) **Do NOT redefine `set` as a delta**: a create has no `before`, `op: upsert` against a missing entity has nothing to assert, and orbital is a declarative intent store.
- **`before` must come from the CLIENT; a server-inferred ancestor is not equivalent.** `Create` reads state when it is *invoked*, not when the author was looking, so an edit landing while someone composes silently becomes the recorded ancestor and the reviewer diffs against a state the author never saw. Only the client knows what it read. If the snapshot read fails and `before` was supplied, creation **hard-errors** rather than downgrading the guarantee to best-effort.
...
- **Orbital's own editor sends `before`, and it sends PRIMITIVES only.** It is the one place that knows what the author was looking at — it already computes `changedOnly` by diffing the edited state against the snapshot it loaded. Edge references are excluded deliberately: they are objects (`{orbId: …}`) living in a different part of the snapshot, so asserting them would compare two shapes rather than two values and could refuse a legitimate proposal. Scalars are also where a silent overwrite actually costs something.
...
Target **end-state**, not a mutation replay — with an **optional per-field precondition**. `set` names the values you want, never a delta; `before` (same keys) names the values you read. Supplying `before` makes the item conditional; omitting it makes it unconditional. See "Preconditions" below. One entity per item; **an edge value may carry only `orbId`/`id`**. A nested entity is rejected at creation with a `400` — see the link-not-deep-write measurement above. `type` is optional on input (resolved from `orbId`, which is `@id` on `ConfigItem` and therefore globally unique) and always present on output. `op` is explicit (`upsert | update | delete`), never inferred: a reviewer approving "update" must not have it silently become "create".
...
| `before` | **JSON Patch's `test` op (RFC 6902)** in mechanism, **`If-Match` (RFC 9110)** in role |

**A worked example** — bumping a server's iDRAC firmware. `type` is absent because `orbId` is `@id` and orbital resolves `IdracSettings` from it; supply `type` only when creating an entity that does not exist yet. `before` is optional, and it is the whole difference between "guarded by the entity-level `base_hash`" and "guarded per field":
...
| **`before`** | **No NetBox equivalent** | NetBox core is last-writer-wins on writes; `prechange_data` is a record, not a precondition. See the settled decision above |

**On the Kubernetes comparison — correct the premise before reusing it.** *"A k8s patch does not require previous values or a version"* is Kubernetes' **default**, not its only mode. It has three opt-in preconditions: `metadata.resourceVersion` **inside the patch body** (enforced by the apiserver), JSON Patch's `test` op (per-path, fails the whole patch), and SSA's `force: false` → 409 against another field manager. So `before` is the same opt-in-precondition pattern at **field** grain instead of object grain — not a foreign concept bolted on.
...
- **Merge:** guarded. `base_values` + `before` + three-way-per-field + atomic pre-check.
...
Two known limits of the shape are filed in `docs/planning/debt.md` § Architecture: `before` guards composite fields at whole-value granularity, and a changeset cannot distinguish asserted intent from incidental fields.
