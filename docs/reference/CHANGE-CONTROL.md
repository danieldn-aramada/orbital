...
- **The gate resolves a mutation's orbIds by FOLLOWING variable references, and descends only through the `orbId` key.** *(Added 2026-09-14.)* `extractResourceIDs` reads literals, `orbId: { eq: $anyName }`, `orbId: { in: [...] }` (literals and variables mixed), and a whole filter object behind `filter: $anyName`. **It must never be widened to "any `{eq: …}` map it finds"**: a filter on `hostname` is not an orbId, and attributing it would put a wrong `resource_id` on an audit row — a wrong id reads as fact, which is worse than a missing one. **Why this is load-bearing for the gate and not just the audit log:** the governing policy is found from the namespace, the namespace comes from the orbId, so an unresolved reference meant a valid variable-form mutation was refused `400 VARIABLE_FORM_REQUIRED` — advising the variable form it was already using. Any client not copying orbital's own `$orbId` spelling, and every compound mutation (which cannot have two variables of one name), was locked out of a governed namespace. **Resolution only ever ADDS orbIds**, so the gate can become stricter but never laxer; an unresolvable reference still yields nothing and is still refused. Do NOT relax that fail-closed default to "fix" a false refusal — resolve the reference instead. **`update` is covered as of 2026-09-14** — it was not at first, because `fetchCurrentState` resolved its target by variable name to bump `version` and capture the audit before-state, so the inline-selector guard refused a renamed-variable update before the gate saw it. The write path now resolves the selector and the patch by reference too (`resolveWriteSelector` / `resolveSetMap`), which is **strictly narrower** than the gate's resolution and must stay that way: the gate only has to KNOW an orbId to find a policy, while the writer must fetch exactly that row, stamp its successor version, and splice a CAS predicate into the query text. So `filter: $whole` resolves for the gate and deliberately not for the writer — there is nowhere to put the predicate, and stamping without guarding is the silent lost update the CAS work exists to prevent.
- **The cascade delete is a compare-and-swap over the WHOLE planned set, parent and children alike.** *(Added 2026-09-15.)* `bulkDeleteGuarded` (`delete.go`) issues a DQL upsert whose `@if` requires every planned node to still hold the version planning saw; an unmet condition deletes nothing and returns `409 MVCC_CONFLICT` naming what moved. **Why it had to change:** the previous delete posted bare uids with no predicate, and `checkDeleteVersion` guards only the PARENT, check-then-act, with planning *and* the approval-gate Postgres round trip inside the window — so a child edited in that window was destroyed with no conflict and nothing to diff back from. DELETE, the irreversible operation, had weaker guarantees than UPDATE. **Do NOT "optimise" the guard into a cardinality check** (`eq(len(all), N)`): that catches nodes disappearing but not nodes being *edited*, which is the case this exists for. Grouping by version value is exact — `func: uid(…)` pins the set so nothing can enter a group, and server-stamped versions only increment, so any edit strictly drops its group's count. **A planned node carrying no `version` fails closed**, because a node that cannot be guarded is the bug, not an exception to it. Pinned by `TestDeleteGuard_ChildEditedAfterPlanningRefusesAndDeletesNothing` (and its positive twin — a guard that refuses everything would pass the first alone).
- **A terminal request's record renders from the stored PAYLOAD (`record`), never from `effect` or a live diff.** *(Added 2026-09-04.)* `effect` is a queue-row summary — counts, plus the single orbId and field when there happens to be exactly one — so a two-entity request collapses to "2 entities / 2 fields" and the per-entity detail is not in the shape at all. A live diff is worse: after a merge it is empty by construction, so the account of what was done vanishes exactly when it becomes the only account. `record` is built server-side from the changeset, one entry per change object, with `applied` folded in from `merge_attempts` (applied in ANY attempt counts — a retry re-attempts only what failed). **Deriving from the payload is what makes it retroactive**: every request already in the database renders correctly with no backfill, which changing `base_effect`'s shape could not do. `before` on each field comes from `base_values`, the ancestor as of the last re-anchor — the value the REVIEWER saw, which is also what merge compares against, and can legitimately differ from `effect.before` (captured at create). **Built server-side, not as a client join of `changes` against `mergeAttempts`** — CLAUDE.md's API-first rule: orbital's UI renders it and so can anyone else's.
...
- **The QUEUE is PostgreSQL-only: it does not compute staleness, and its `stale`/`subtreeChanged`/`staleEntities`/`missingTargets` are ABSENT, not `false`.** *(Added 2026-09-15.)* Rendering every row through `State` was an N+1 against DGraph — **measured at 1,222 DGraph queries and 1.21s for one unfiltered page load** of 507 requests, of which 506 were terminal and discarded the result immediately. The list now renders from `storedState` (PostgreSQL alone): **0 queries, 0.009s.** Three rungs, cheapest first — `storedState` (list) → `State` (+scope and version vector, detail view) → `StateWithSnapshot` (+content, diff and merge). **GitHub draws the same line**: `mergeable` is returned by the single-PR endpoint and never by the list. **Absent is not false** — a `stale: false` asserts "not stale" about a question the endpoint no longer asks, which is a wrong answer rather than a missing one; clients must read a missing `stale` as "open the request to find out". **`availableActions` omits `merge` and the rebase-`edit` on list items** for the same reason: they are the only actions whose eligibility depends on staleness. `approve` does not, so `awaiting_review` stays exact. **Known, accepted divergence:** a list item's `approvals` counts decisions cast against the current changeset REVISION, while the detail view additionally requires the scope hash to match — so the queue can show one more approval than the detail page for a request whose entity moved after approval. That window is exactly what the deferred "should the queue show staleness, from a cache" question covers (`backlog.md`). Pinned by `TestQueueList_MakesNoDGraphCallsAtAll`, which routes DGraph to a trap server that fails the test if contacted — a counter would let a regression through at "only a few".
...
- **The unfiltered queue page** renders every row. The default tab narrows in SQL first, so only "All" is expensive, and the owned-subtree expansion behind each row is now a single batched round-trip (`collectRelatedOrbIDsBatch`) rather than one per orbId. What remains — one round-trip per row, and the `change_target` projection table held in reserve — is tracked in `docs/planning/debt.md`.
