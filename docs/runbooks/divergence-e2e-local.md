...
- **MVCC `version` check** can reject the mutation if the DGraph node's `version` field changed since the divergence was captured. For local testing, this is rare but possible if you re-trigger imports between divergence capture and accept.
