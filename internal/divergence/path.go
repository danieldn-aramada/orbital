...
const (
	...
	// Suffix is the file extension for report files.
	Suffix = ".json"
)

// ReportKey builds the canonical S3/blob key for a given OCI repo path and
// timestamp string. The timestamp must already be in the RFC3339-colon-replaced
// format (callers can use TimestampForKey).
func ReportKey(repoPath, ts string) string {
...
// report filenames. Lexicographic ordering of these filenames matches
