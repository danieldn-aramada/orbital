...
func newEventHandler(t *testing.T) *handler.AuditHandler {
	t.Helper()
	t.Chdir("../..")
...
func clearEvents(ctx context.Context) {
	for attempt := 0; attempt < 5; attempt++ {
		_, _ = testDB.AuditEventResourceType.Delete().Exec(ctx)
		_, _ = testDB.AuditEventResource.Delete().Exec(ctx)
		if _, err := testDB.AuditEvent.Delete().Exec(ctx); err == nil {
			return
		}
...
func createEvent(t *testing.T, actor string, operations, resourceTypes, resourceIDs []string, category string) {
	...
	c := testDB.AuditEvent.Create().SetActor(actor).SetOperations(operations)
	if category != "" {
		c = c.SetEventCategory(category)
	}
	ev := c.SaveX(ctx)
	for _, rid := range resourceIDs {
		testDB.AuditEventResource.Create().SetOrbID(rid).SetAuditEventID(ev.ID).ExecX(ctx)
	}
	for _, rt := range resourceTypes {
		testDB.AuditEventResourceType.Create().SetResourceType(rt).SetAuditEventID(ev.ID).ExecX(ctx)
	}
}
...
func TestEventList_OrderedByTimestampDesc(t *testing.T) {
	...
	e1 := testDB.AuditEvent.Create().
		SetActor("order-test").
		SetOperations([]string{"op"}).
		SetTimestamp(time.Now().Add(-2 * time.Minute)).
		SaveX(ctx)
	testDB.AuditEventResource.Create().SetOrbID("test:srv").SetAuditEventID(e1.ID).ExecX(ctx)
	testDB.AuditEventResourceType.Create().SetResourceType("Server").SetAuditEventID(e1.ID).ExecX(ctx)

	e2 := testDB.AuditEvent.Create().
		SetActor("order-test").
		SetOperations([]string{"op"}).
		SetTimestamp(time.Now()).
		SaveX(ctx)
	testDB.AuditEventResource.Create().SetOrbID("test:srv").SetAuditEventID(e2.ID).ExecX(ctx)
	testDB.AuditEventResourceType.Create().SetResourceType("Server").SetAuditEventID(e2.ID).ExecX(ctx)
