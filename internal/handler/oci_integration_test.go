...
func newOCIHandler(t *testing.T, keyPath string) *handler.OCI {
	...
	return handler.NewOCI(testDB, cfg, scratchExportDir, slog.Default(), 30*time.Second, nil)
}
...
func TestOCIPublish_UnconfiguredReturns503(t *testing.T) {
	...
	h := handler.NewOCI(testDB, oci.Config{}, scratchExportDir, slog.Default(), 30*time.Second, nil)
