...
func main() {
	...
	// INCOMPATIBLE with OIDC bearer auth — go-oidc's JWKS keyset
	...
	if fips140.Enabled() {
		slog.Warn("fips140 ENABLED — crypto restricted to FIPS-approved algorithms. INCOMPATIBLE with OIDC bearer auth: go-oidc JWKS verification uses SHA-1 and will PANIC the process on first token verification. Do not combine fips140=only with bearer auth until go-oidc is FIPS-clean.", "fips140", true)
	} else {
