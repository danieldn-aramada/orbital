...
- **GraphQL endpoint is `/api/v1/graphql` in both apps** — orbital and orb both register the proxy at `/api/v1/graphql` so shared client code in `shared.js` works identically. Do NOT register orb's GraphQL at `/graphql` or any other path; client code does not branch per-app. `GraphQLPath` in `layout.UIConfig` must match the registered route.
