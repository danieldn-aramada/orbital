...
- **go-keyring on macOS uses `security -i` (legacy keychain API)** — this does NOT trigger Touch ID/fingerprint. It shows an "allow/deny" dialog on first access per binary. Touch ID would require CGo + LocalAuthentication framework — not implemented, not needed for internal tooling.
