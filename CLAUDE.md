...
| **Change requests, approval policies, the write gate, propose-from-editor** | `docs/reference/CHANGE-CONTROL.md` (landscape evidence: `change-control-research.md`) |
...
| Planning or starting any spike | `docs/planning/backlog.md` (definitions) → `docs/spikes/` (designs in flight) |
...
- **Multi-step work starts with an acceptance list, and I wait for your OK before implementing.** Any task that is 3+ steps, a new feature, a spike, or touches a security/data guarantee: post a NUMBERED acceptance list first — one line per observable behaviour that must hold when the work is done — then stop until you approve it. A wrong or missing item is cheapest to fix before any code exists. Skip the ceremony for typos, copy changes, and one-line fixes.
  - **Tests are transcribed FROM that list, one item to one test name, before any test body is written.** Never author tests by describing what the code already does — a test written from an implementation can only confirm that implementation, so it inherits the gap instead of catching it.
  - **Before reporting done, walk the list and name the test covering each item.** "The suite is green" is a different claim from "the list is satisfied", and they diverge silently when the list and the tests were written independently.
  - **A code comment asserting a property is not evidence of it.** Spike 36 shipped a bypass whose comment read "allowed and recorded as such" directly above a `logger.Warn` — every re-read confirmed the claim instead of checking it. If a comment states a guarantee, something must assert it.
  - *Why:* the Spike 36 session-2 plan required "audit event flagged privileged" in three separate places. The test was written afterwards from the code, asserted only "no error" and "the value changed", and the audit half vanished without a trace. Suite green, gate reported satisfied, guarantee absent.
  - **This does NOT mean every item becomes a permanent test** — it composes with the calibration rule below. The list says what must be true; the calibration rule decides how each item is *verified*. An item may be discharged by a live check against the local stack or a visual confirmation, and the walk-the-list step then names that instead of a test. What is never acceptable is an item with nothing behind it at all.
...
- **Any persistence requires a round-trip test — and audit/observability writes ARE persistence.** If data is written to disk, PostgreSQL, or any file: write a test that writes, **reads back through the path a consumer would actually use**, and asserts. Persistence bugs are invisible without this. E.g., a new `bool` field on a struct written to JSON must be verified to survive encode+decode. This is a concrete regression class (silent serialization loss) — it earns the test by default.
  - **The rule covers anything written for someone to read LATER**: audit-event fields, provenance stamps, retention markers, job records. These look like logging, so the rule gets skipped — that misclassification is the bug. Spike 36 shipped a `bypass_roles` privileged write whose only record was a `logger.Warn`; the plan required it on the audit event in three places, the code asserted "recorded as such" in a comment, and nothing read it back. It was invisible until someone asked.
  - **Read back through the consumer API, not the write path.** Grepping the process log proves the log line, not the audit row. If the guarantee is "an operator can find this in `GET /api/v1/audit-log`", the test queries `GET /api/v1/audit-log`.
  - **Assert the negative too.** A flag that fires when nothing happened is worse than no flag — it trains whoever reads the record to ignore the field. Test both that it appears when it should and that it is absent when it should not.
  - **Why these specifically:** every other kind of bug announces itself — a refusal 403s, a merge deadlocks, a page 500s. A missing audit row changes nothing observable at the time, and is only discovered when someone asks a question that can no longer be answered.
...
- **API-first means orbital's own UI is a thin renderer.** Anything the orbital UI displays, an upstream client (AEP, orbctl, a third party) must be able to display with the same ease — so response shapes carry what the view needs, and the UI does not compute it. **Bespoke client-side logic in orbital's UI is a signal the API isn't carrying its weight** — fix the response, don't add a walker to the JS. Real burn: the export-preview response was a nested tree, so `orbital.js` had to flatten it; that same walk would have been re-implemented by every integrator. Flattened 2026-08-24. Corollary: orbital's UI is a **first-class consumer of orbital's public API**, never a privileged server-rendered path.
- **`CHANGELOG.md` at repo root is the source of truth for release notes — NOT the GitHub release page.** [Keep a Changelog](https://keepachangelog.com) format, `## [Unreleased]` appended as work lands. **The deciding reason is air-gap:** orbital ships to disconnected environments, and an operator holding the tarball with no network must still be able to read what changed — release notes that live only on a website contradict the product's own premise. Two supporting reasons: the repo is migrating orgs and Release objects do not always survive that, and orbital has three tag lineages (`v*` / `cli/v*` / `orb/v*`) that a single GitHub release list flattens by date. GitHub Release bodies are **generated from the file**, never the reverse. This reverses an earlier "no CHANGELOG until versioned releases ship" decision — releases have been shipping since v0.0.1 and that precondition went unnoticed. Majority practice among comparable projects keeps the file in-repo (Kubernetes `CHANGELOG/`, Rust `RELEASES.md`, Django `docs/releases/`, Terraform, Prometheus).
- **`ROADMAP.md` is a ONE-PAGER: milestones, not work items.** It carries a **Timeline** gantt (the arc + current phase, at the top — it is the only element showing shape over time, so it orients before the detail does), a **Status** table, and **Next up** (one line each, linking out). **No "recently shipped" section** — a roadmap is forward-looking by convention, and shipped work belongs in `CHANGELOG.md` alone; two records of the same thing drift. **An entry that needs a paragraph does not belong there** — spike definitions live in `docs/planning/backlog.md`, designs in `docs/spikes/`, technical debt in `docs/planning/debt.md`. Split 2026-08-28, when ROADMAP had reached 233 lines and 57% of it was debt + spike essays, burying the milestone view it exists to give. The audience is a principal engineer or manager asking "where are we"; anything they'd scroll past belongs elsewhere. Every comparable project converged on this shape (Kubernetes KEPs, Rust RFCs, Django DEPs — thin strategic doc + one document per feature + tracker for debt).
- **Filenames: ALL-CAPS only at repo root, kebab-lowercase everywhere else.** `README.md`, `ROADMAP.md`, `CLAUDE.md` are caps because uppercase sorts first in `ls` and GitHub surfaces them — a real convention with a mechanical reason. `docs/reference/*.md` is the one deliberate exception: caps signals "domain authority." Everything else (`docs/planning/debt.md`, `docs/spikes/spike-27-atlas-migrations.md`, `docs/testing-strategy.md`) is lowercase. Caps in a subdirectory is cargo-culting the look without the reason, and reads as machine-generated.
- **Spike docs live in `docs/spikes/` and are WIP. When a spike closes, fold its decisions into `docs/reference/<DOMAIN>.md` and DELETE the spike doc.** `docs/spikes/spike-N-*.md` means "open question under investigation — may be wrong"; carry a `**Status:**` line (KEP/ADR convention) so a partially-shipped spike says so. Leaving a *settled* design under a spike name is a status lie: Spike 11 shipped 2026-06-05, and by the time it was folded (2026-08-24) its doc still specified two flat roles and a DGraph `@auth` service token that `reference/AUTH.md` explicitly forbids — a future session could have implemented the forbidden thing. Do **NOT** promote a finished spike to a standalone `docs/<topic>-design.md` — that recreates the `docs/decisions/` ADR discoverability gap removed 2026-07-07, just with a new filename. **One entry point per domain:** decisions inline into the domain reference doc; oversized material (sequence diagrams, algorithm walkthroughs) may remain a companion file **only if the domain doc links it**. git history preserves the deliberation.
...
- **Swagger bodies must be self-documenting typed DTOs, never `@Param body body object`** — external services consume these APIs; a loose `object` renders as an empty `{}` with no field docs. Always: (1) define a named DTO (e.g. `exportRequest`) and reference it `@Param body body exportRequest true "..."`; (2) give every field an `example:"..."` tag + doc comment (these become the Example Value + descriptions); (3) mark required fields with `validate:"required"` — adds them to the spec's `required` array (red asterisk). The `validate` tag is **documentation-only**: no validator runs, the handler's own `if x == "" { 400 }` check is the real enforcement — use `validate`, not `binding` (Gin convention; this is Echo). Same rule applies to responses: prefer named types over `{object} map[string]string`. Request bodies converted 2026-07-23 (export/restore/import/backup); response-model sweep (the ~65 `{object} map[string]string` annotations) is tracked in ROADMAP Post-MVP.
...
- **`orbId` is always conventional, never random: `<namespace>:<kind>-<natural-key>`** — `kind` = the kebab-case ConfigItem type (`network-device`, `network-adapter`, `network-interface`, …); `natural-key` = the entity's stable real-world identifier(s) — a device serial, or an owner key + local id (serviceTag + Redfish FQDD; device serial + port). **Never a UUID/random value** — a derivable orbId is what makes upserts idempotent and lets clients construct ids without a lookup. `orbId` is **`@id(interface: true)`** on the `ConfigItem` interface as of **schema v8 (2026-09-17)**, so cross-type uniqueness is now enforced by DGraph and not only by the `<kind>-` prefix convention — a second type reusing an orbId is refused (*"already exists for field orbId in some other implementing type of interface ConfigItem"*). Before v8 it was a bare `@id`, which DGraph scopes **per implementing type**. ⚠️ **The constraint is NOT retroactive** — the schema alter succeeds with duplicates already stored, without scanning or rejecting them, so applying it to a graph never proves that graph is clean. Run the audit in `docs/reference/DGRAPH.md` § orbId convention before applying it anywhere and after any bulk import; blue passed 2064/2064 with 0 collisions before v8. **Keep following the prefix convention regardless** — it is what makes an orbId readable and derivable, and the constraint is a backstop, not a substitute. **Adding a new ConfigItem type REQUIRES defining its orbId convention** (per-type table in `docs/reference/DGRAPH.md`; checklist in `docs/playbooks/add-configitem.md`). Network types adopted this 2026-08-11 (`network-device-`/`network-adapter-`/`network-interface-`); **Server adopted `server-<serviceTag>` 2026-08-12** (natural-key = the vendor's stable chassis identifier — **Dell: Redfish `ComputerSystem.SKU`**, i.e. the Service Tag; **Supermicro: `ComputerSystem.SerialNumber`**. **NOT Dell's `SerialNumber`**, which is a different value on 15G+ — R650 reports SKU `CFRHDX3` and SerialNumber `MXFC400359006Z`; `Server.serialNumber` stores the latter and is never an identity key. **Never Redfish `AssetTag`**, which is org-assigned and can be empty → breaks scan-idempotency); Rack/IPAddress still use legacy bare keys and migrate when next touched — do NOT special-case network types as the exception, the whole graph is meant to converge on this.
