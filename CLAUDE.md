...
- **`orbId` is always conventional, never random: `<namespace>:<kind>-<natural-key>`** — `kind` = the kebab-case ConfigItem type (`network-device`, `network-adapter`, `network-interface`, …); `natural-key` = the entity's stable real-world identifier(s) — a device serial, or an owner key + local id (serviceTag + Redfish FQDD; device serial + port). **Never a UUID/random value** — a derivable orbId is what makes upserts idempotent and lets clients construct ids without a lookup. `orbId` is `@id` on the `ConfigItem` interface, and the `<kind>-` prefix exists so ids stay unique **across** types. ⚠️ **That cross-type uniqueness is a CONVENTION, not a constraint DGraph enforces** — corrected 2026-09-03. `@id` does keep values unique, but **per implementing type**; DGraph's docs say so on the `@id` page itself: *"By default, if used in an interface, the `@id` directive will ensure field uniqueness for each implementing type separately … this allows two different types implementing the same interface to have the same value for the inherited `@id` field."* Reproduced here: a `Rack` and a `Server` both accepted the same `orbId`; a second `Rack` with it was refused (*"already exists for field orbId inside type Rack"*). `@id(interface: true)` enforces it globally but is **not retroactive**. **Keep following the convention — it is what keeps the graph correct today** (audited: 2064 distinct orbIds, 0 collisions). Blast radius + audit query in `docs/planning/debt.md`. **Adding a new ConfigItem type REQUIRES defining its orbId convention** (per-type table in `docs/reference/DGRAPH.md`; checklist in `docs/playbooks/add-configitem.md`). Network types adopted this 2026-08-11 (`network-device-`/`network-adapter-`/`network-interface-`); **Server adopted `server-<serial>` 2026-08-12** (natural-key = **Redfish System SerialNumber** — Dell Service Tag / Supermicro `S…`; **never `asset_tag`**, which is org-assigned and can be null → breaks scan-idempotency); Rack/IPAddress still use legacy bare keys and migrate when next touched — do NOT special-case network types as the exception, the whole graph is meant to converge on this.
